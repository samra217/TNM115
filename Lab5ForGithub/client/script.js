...
function generateArtistText(artistJsonObject, imgElement) {
    ...
    document.getElementById("discogs").appendChild(createContentElement("p", "Discogs URL"));
...
function realName(json) {
    if(json.realname != null) {
        document.getElementById("real-name").innerText = "Real name:  " + json.realname;
    }
}
...
function memberInGroups(json) {
    if (json.memberInGroups != null) {
        ...
        for (let i = 0; i < json.memberInGroups.length; i ++) {
            ...
            const formatString = (memberIn.activeInGroup === true) ?  "Active in: " + memberIn.name : "Not active in: "+memberIn.name; 
...
function createHomeButton() {
    const buttonDiv = document.createElement("div");
    const button = createContentElement("button","Home");
    button.onclick = generateHomePage();
